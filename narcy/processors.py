...
def relation_to_record(r):
    ...
    return Record(
        ...
        head_lemma=head._.lead._.lemma,
        sub_lemma=sub._.lead._.lemma,
        head_neg=head_neg,
        sub_neg=sub_neg,
        head_pos=head_pos,
        head_dep=head_dep,
...
def get_appos(relations):
    ...
    for relation in relations:
        ...
        if not subj._.drive._.is_semantic or not subj._.drive._.is_appos:
            continue
...
def get_all_relations(doc):
    svos = doc_to_svos_df(doc)
    locs = doc_to_locs_df(doc)
    appos = doc_to_appos_df(doc)
    all_dfs = [svos, locs, appos]
